// ui.js
import { GAME_X, GAME_WIDTH, LEFT_UI, RIGHT_UI, SAFE, dpad, jumpBtn, restartBtn, startMenuBtn, centerFullBtn } from './utils.js';
import { touch } from './input.js';
import { player } from './player.js';
import { gameState, currentLevelIndex, totalPlayTime, stateTimer, platforms, spikes, stars } from './level.js';

export function drawGameArea(context) {
  context.fillStyle = "#1e293b";
  context.fillRect(GAME_X(), 0, GAME_WIDTH(), canvas.height);
}

export function drawGround(context, canvas) {
  context.fillStyle = "#334155";
  context.fillRect(GAME_X(), canvas.height - 40, GAME_WIDTH(), 40);
}

export function drawControlsBackground(context, canvas) {
  context.fillStyle = "#111";
  context.fillRect(0, 0, LEFT_UI(), canvas.height);
  context.fillRect(canvas.width - RIGHT_UI(), 0, RIGHT_UI(), canvas.height);
}

export function drawDpad(context, canvas) {
...
export function drawJumpButton(context, canvas) {
...
export function drawRestartButton(context, canvas) {
  const x = canvas.width - RIGHT_UI() / 2;
  const y = 60;
  context.save();
...
export function drawMenuButtons(context, canvas) {
...
export function drawFog(context) {
...
export function drawUI(context, canvas) {
  context.fillStyle = "white";
  context.font = "bold 16px Arial";
  context.textAlign = "center";
  ...
  if (gameState === "menu") {
    ...
    drawMenuButtons(context, canvas);
